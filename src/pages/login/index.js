...
export default class Login extends PureComponent {
  ...
  render() {
       ...
       if ( isForgetPwd ) {
        ...
        return (
          <div style={{margin: "0 0 20px 25px"}}>
            <Button
              type={isVerification ? "link" : "text"}
              className={cx('verification-btn')}
              onClick={() => {
                this.setState({
                  isVerification: true,
                },() => {
    ...
    const registerBoxStyle = () => {
      if (isVerification) {
        return {
          height: isForgetPwd ? '480px' : '380px'
        }
      } else {
        return {
          height: '400px'
      ...
      <div className={cx('login')}>
        <Row className={cx('center-box')}>
          ...
          <Col span="10" className={cx('registerBox')}>
            <div className={cx('register_box')} style={registerBoxStyle()}>
              <h3 style={{marginLeft: 25}}>{ isForgetPwd ? "GET BACK." : "LOG IN."}</h3>
              ...
              <div className={cx('login-box')}>
                ...
                >
                  {renderButton()}
                  {renderForm()}
                  {isVerification && !isForgetPwd && (
                    <div className={cx('register-btn')}>
                      <Button onClick={this.forgetPwd} type="text">忘记密码</Button>
                    </div>
                  )}
