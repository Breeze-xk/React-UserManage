  ...
  const handleSubmit = () => {
    form
      ...
      .then((values) => {
        ...
        if (values.specificationConfig != undefined) {
          let dataArr = [];
          values?.matingInformation?.forEach((element, i) => {
            console.log(element);
            console.log(element.acreage);
            if (
              element.acreage != undefined &&
              element.matching != undefined &&
          ...
          if (dataArr.length > 0) {
            values.supportingInfo.specificationConfig = dataArr;
          }
        }
    ...
    >
      <Form
        ...
        initialValues={{
          ...
          supportingInfo: {
            // averageLayerMax: 1,
            // averageLayerMin: 1,
            // groundStoppingRatio: 1,
            // perCapitaSiteArea: 1,
            // residentialParking: 1,
            // supportingParking: 1,
            // undergroundParkingEfficiency: 1,
            // worthOfPopulation: 1,
          },
          heighlimitType: 'HEIGH', // 产品高度限制类型 HEIGH 高度 FLOOR 层数
          specificationConfig: [
            ...
            //   maxHouseWidth: '', // 最大面宽
            //   rotationAngle: '', // 旋转角度
            //   widthlimitTypeFlag: false, // 面宽设置
            //   widthlimitType: 'AREA', // 是否选择面宽范围
            //   faceWideRangeSetting: { // 不同点 后端要数组形式---------------
            //     areaWidthLimit: ['', ''], // 产品面宽限制[最小值, 最大值]
            //     heighRatio: '',
            //     heighRatioCheck: false,
            //     areaRatio: '',
      ...
      >
        <Row  gutter={[32, 32]}>
          ...
          <Col span={10}>
            <Form.Item label="城市编码" name="countryCode">
              <Input style={{ width: '180px' }} placeholder="请输入城市编码" />
            </Form.Item>
          </Col>
        ...
        <Row>
          <Radio.Group defaultValue={'decimal'} onChange={aryOnChange}>
        ...
        <Row>
          <Col span={14}>
            <Radio.Group onChange={onChanges} value={radioValue}>
              ...
              >
                ...
                >
                  {!householdChecked && (
                    <InputNumber min={0} style={{ width: '180px' }} controls={false} placeholder="请输入住宅停车" />
                  )}
                </Form.Item>
              </div>
              <div
              ...
              {householdChecked && (
                <Form.List className={cx('formList')} name="matingInformation">
                  {(fields, { add, remove }) => (
                    <>
                        ...
                        >
                          <Form.Item
                            ...
                            rules={[
                              ({ getFieldValue }) => ({
                                validator(_, value) {
                                  let arr = getFieldValue().matingInformation;
          ...
          <Col span={10}>
            ...
            <Form.Item
              ...
              hasFeedback
            >
              <InputNumber min={0} controls={false} style={{ width: '180px' }} placeholder="请输入地面平车比(%)" />
            </Form.Item>
            <Form.Item
              {...formItemLayout}
              label="地面停车效率(㎡/辆)"
            ...
            >
              <InputNumber min={0} controls={false} style={{ width: '180px' }} placeholder="请输入户均人口" />
            </Form.Item>
          </Col>
        </Row>
      </Form>
